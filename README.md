...
- 🧭 **Browser Agent**: Added `fillText` macro (focus + type + retries), hover/scroll tools + double-click support, auto-repair for invalid JSON tool outputs, and improved Google Docs/Slides typing verification (includes `snapshot.axText`)
- 🖥️ **Browser Agent Click**: More reliable click targeting via hit-testing (reduces “click didn’t land” on dynamic sites)
- 🤖 **Gemini**: Added `gemini-3-flash-preview` model option
...
- On Google Slides/Docs, entering text-edit mode can still be finicky; try `fillText` (macro), precise clicks, `click` with `count=2`, or `press` → `Enter` before `type`
